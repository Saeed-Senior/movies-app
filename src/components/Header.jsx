...
<nav className='flex justify-center'>
            <ul className={burger ? 'h-screen py-8 flex flex-col items-center justify-between fixed z-20 bg-gray-900/90 w-full' : "fixed flex justify-between items-center sm:absolute z-20 py-8 px-3 bg-gray-900/50 w-full"}>
               ...
               <li onClick={() => handleClickInfo()} className={info ? 'p-2 bg-black/70 rounded' : ''}>
                  ...
                  <li className="cursor-pointer"><img className="max-w-[18px]" src={smsImg} alt="img" /></li>
                  <li className={burger ? 'link-toggle' : "cursor-pointer link-main"}>Sign Up <span className={info ? "" : 'hidden'}>&#128512;</span></li>
                  </ul>
                  <span className={info ? "text-[8px]" : 'hidden'}>These components are not in working order, work is underway!</span>
               </li>
